...
class ChunkData(NamedTuple):
    """
    A named tuple to store the keyword and data of a text chunk.
    """
...
class PNGExtractor:
    ...
    def __init__(self, png_path: str, chunk_type: str | list[str] | None = None):
        self.png_path = png_path
        if chunk_type is None:
            self.chunk_types: list[bytes] = [b"tEXt", b"zTXt", b"iTXt"]
        if isinstance(chunk_type, list):
            self.chunk_types: list[bytes] = [c.encode() for c in chunk_type]
        if isinstance(chunk_type, str):
            self.chunk_types: list[bytes] = [chunk_type.encode()]
        self.chunks: dict[str, ChunkData] = dict()
    ...
    def decode_chunk_data(chunk_type: bytes, data: bytes) -> ChunkData:
        ...
        try:
            if chunk_type == b"tEXt":
                ...
                keyword_bytes, rest = data.split(b"\x00", 1)
                ...
                keyword_bytes, compression_flag, rest = parts[0], parts[1], parts[2]
                if compression_flag in (b"0", b""):
                    text_bytes = rest.split(b"\x00", 1)[1]
                else:
                    compression_method = rest[0]
            ...
            keyword = keyword_bytes.decode("utf-8") if keyword_bytes else "unknown"
            text = text_bytes.decode("utf-8") if text_bytes else data.decode("utf-8")
            return ChunkData(keyword, text)

    def _get_chunk_meta(self, f: Any) -> tuple[int, bytes | None]:
        """
        ...
        chunk_length: int = struct.unpack("!I", length_bytes)[0]
        chunk_type: bytes = f.read(4)
        ...
        with open(self.png_path, "rb") as f:
            ...
            while True:
                ...
                if chunk_type in self.chunk_types:
                    logger.debug(
                        f"{chunk_type.decode('utf-8')} chunk found ({chunk_length} bytes)"
                    )
